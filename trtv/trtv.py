...
def TRTV(number,tvshowmovie):
	trtv = OpenURL("https://trailers.to/en/newest/"+tvshowmovie+"?pageNumber="+str(number)).replace("\n","").replace("\r","")
	trtvre = re.compile("figure.{1,5}href=.{1,5}en\/(movie|tvshow)\/(\d+).+?(tt\d{3,15})").findall(trtv)
	#fim={}

...
def ListSeries():
	...
	prog = 1
	cPagetmdbgenero = '10765' if cPagetmdbgenero == "878" else cPagetmdbgenero
...
def DownloadTrtv():
	...
	tmdb = str(meta["tmdb_id"])
	imdb = meta["imdbnumber"]
	id = trtv.RetIdMovies(imdb)
	mdbj = mg.get_tmdb_details(tmdb_id=tmdb, imdb_id="", tvdb_id="", title="", year="", media_type="movies", preftype="", manual_select=False, ignore_cache=False, lang="en")
	if id:
		titulo = "M"+str(meta["tmdb_id"])+" "+id+" "+xx.remove_accents(mdbj["title"])+".mp4"
...
def tvshow_recom():
	...
	entries = json.loads(mdb)
	if entries['results']:
		RP = 'plugin://plugin.video.CubeTor/?mode=tmdb.tvshow_recomShow&url='+quote_plus(url)
		xbmc.executebuiltin('ActivateWindow(10025,"'+RP+'")')
	else:
		xx.NF("Não há recomendações")
def tvshow_recomShow():
	mdb = xx.OpenURL('https://api.themoviedb.org/3/tv/'+url+'/recommendations?api_key=bd6af17904b638d482df1a924f1eabb4')
	entries = json.loads(mdb)
	#ST(entries['results'])
	#return
...
#----------------------------------------
def InTheaters():
#def InTheaters(page="upcoming",region="br"):
	region=eval(dados)['region']
	if url == "now_playing" and region=="br":
		xx.AddDir("Nos cinemas (EUA)", "now_playing", "tmdb.InTheaters", isFolder=True, dados={"region": "us"})
		xx.AddDir("Próximos Lançamentos", "upcoming", "tmdb.InTheaters", isFolder=True, dados={"region": "br"})
	if url == "now_playing" and region=="us":
		xx.AddDir("Próximos Lançamentos (EUA)", "upcoming", "tmdb.InTheaters", isFolder=True, dados={"region": "br"})
	trak = xx.traktM()
	progress = xbmcgui.DialogProgress()
	progress.create('Carregando...')
	progress.update(0, "Carregando...")
	prog = 1
	link = xx.OpenURL('https://api.themoviedb.org/3/movie/'+url+'?api_key=bd6af17904b638d482df1a924f1eabb4&language=en-US&page=1&region='+region)
	entries=json.loads(link)
	for entry in entries['results']:
		if (progress.iscanceled()): break
		progtotal = int( 100*prog/len(entries['results']) )

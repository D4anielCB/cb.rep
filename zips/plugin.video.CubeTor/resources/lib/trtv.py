...
def Categories():
	...
	xx.AddDir("[COLOR yellow]Favoritos[/COLOR]", "", "trtv.Favoritos", isFolder=True)
def Manual():
	q = xbmcgui.Dialog().input("Coloque a url do filme/seriado do trailers.to")
	if not q: return
	#q = "https://trailers.to/en/movie/2086791/shang-chi-and-the-legend-of-the-ten-rings-2021"
	...
	try:
		link = xx.OpenURL(Cproxy+q)
	except:
		xx.NF("Erro ao abrir o site")
		return
...
def Manual_Movie(imdb,code):
	...
	mm = mg.get_tmdb_details(tmdb_id="", imdb_id=imdb, tvdb_id="", title="", year="", media_type="movies", preftype="", manual_select=False, ignore_cache=False)
	#ST(mm)
	xx.AddDir(mm["title"]+ " ([COLOR blue]Play[/COLOR])", code, "trtv.PlayUrlTv", isFolder=False, IsPlayable=True, dados={'mmeta': mm, 'titulo': titulo})
	xx.setViewM2()
def Manual_TvShow(imdb,code):
	...
	xx.AddDir(meta[-1]["TVShowTitle"]+ " ([COLOR blue]Play[/COLOR])", code, "trtv.SeasonsPlay", isFolder=True, dados={'meta': meta[-1]})
	xx.setViewS()
	#ST(externalj)
#---------------------------------------
def SeriesFolder():
	...
	for entry in entries:
		try:
			if not entry[2] in check:
				...
				pc = 1 if int(entry[2]) in trak else None
				xx.AddDir(meta[int(entry[2])]["name"], url, playordownload, isFolder=True, dados={'meta': metasea, 'season': entry[2], 'pc': pc})
...
def Episodes(playordownload="trtv.PlayFile"):
	...
	lastepisode = trakt.LastEpisode(dados2["meta"]["imdbnumber"])
	#ST(lastepisode)
	#return
	xx.AddDir("Series", "", "trtv.SeriesFolder", isFolder=True)
	prog = 1
	...
	for entry in entries:
		...
		try:
			...
			pass
		if lastepisode[0] == int(season) and lastepisode[1] == int(entry[3]):
			break
	progress.close()
def PlayFile():
	dados2 = eval(dados)
	...
	if not Addon.getSetting("cDownload") in File:
...
def DownloadMP4(titulo="",url2="",ask=False):
	...
	try:
		...
		xx.NF("Erro ao abrir trailers.to")
		return
	...
	download.StartDownload(u,file,"https://trailers.to",url2)
def ResumeFile():
...
def PlayUrlMovie(D=""):
	...
	return
	year = ""
	mdb = xx.OpenURL("https://api.themoviedb.org/3/movie/"+url+"?api_key=bd6af17904b638d482df1a924f1eabb4&language=en&append_to_response=external_ids")
	mdbj = json.loads(mdb)
	imdb = mdbj["external_ids"]["imdb_id"]
	#ST(mdbj)
	#mdb["release_date"].split("-")[0]
	if "release_date" in mdbj:
		year = " "+mdbj["release_date"].split("-")[0]
	q = re.sub('[^A-Za-z0-9]+', ' ', mdbj["title"])
	#ST(year)
	id = ""
	try:
		git = eval(xx.OpenURL("https://raw.githubusercontent.com/D4anielCB/cb.rep/master/trtv/movies.txt"))
		id=git[imdb]
	except:
		...
		if entries:
			id = entries[0]
	if not id:
		xx.NF("Não encontrado - usando Google")
		id = google.BuscaTrtvMovie(mdb)
		if not id:
			xx.NF("Não encontrado")
			return
	PlayPrevia(id)
def PlayPrevia(id):
	#xx.PlayUrl("","https://c-2.notacdn.ru/_.m3u8",True)
	#return
	dados2 = eval(dados)
	#ST(id)
	try:
...
def Favoritos():
	try:
		link = xx.OpenURL("https://raw.githubusercontent.com/D4anielCB/cb.rep/master/favoritos.trailers.to")
		#ST(link)
		...
		for name2,id2,url2 in lista:
			...
			try:
				...
				meta = mg.get_tvshow_details(title=name2, tmdb_id=id2, ignore_cache=MUcache, lang=MUlang)
				url3 = re.compile("\/(\d+)\/").findall(url2)
				#xx.AddDir(meta[-1]["TVShowTitle"], url2, "trtv.SeasonsPlay", isFolder=True, dados={'meta': meta[-1]})
				xx.AddDir(meta[-1]["TVShowTitle"]+ " ([COLOR blue]Play[/COLOR])", url3[0], "trtv.SeasonsPlay", isFolder=True, dados={'meta': meta[-1]})
				if cDirtrtvDown:
					xx.AddDir(meta[-1]["TVShowTitle"]+ " ([COLOR yellow]Download[/COLOR])", url3[0], "trtv.Seasons", isFolder=True, dados={'meta': meta[-1]})
			except:
				pass
		progress.close()
	except:
		pass
#----------------------------------------
def RetIdMovies(imdb):
	link = xx.OpenURL(Cproxy+"https://trailers.to/watch/imdb/"+imdb)
	ids =  re.compile("en\/movie\/(\d+)").findall(link)
	return ids[0]
def RetId(imdb):
	link = xx.OpenURL(Cproxy+"https://trailers.to/watch/imdb/"+imdb)
	ids =  re.compile("en\/(movie|tvshow)\/(\d+)").findall(link)
	if "Coming up soon!" in link:
		meta = eval(dados)["meta"]
		year = ""
		if "premiered" in meta:
			year = " "+meta["premiered"].split("-")[0]
		trtvpage = xx.OpenURL(Cproxy + "https://trailers.to/en/popular/movies-tvshows-collections?q="+quote_plus(meta["TVShowTitle"]+year))
		entries = re.compile("en\/tvshow\/(\d+).{300,600}"+meta["imdbnumber"]).findall(trtvpage.replace("\r","").replace("\n",""))
		return entries[0]
	else:
		return ids[0][1]
#----------------------------------------

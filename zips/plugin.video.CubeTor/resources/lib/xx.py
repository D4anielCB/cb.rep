	...
	if "mmeta" in dados2:
		try:
			if CtraktAuth and CTraktResume:
				RSM = trakt.TraktResumeM(dados)
				#ST(RSM)
				if RSM[0] == -1:
					d = xbmcgui.Dialog().yesno("CubeTor", "Não foi possível resgatar o progresso, reproduzir assim mesmo?")
					if not d:
						return
				elif RSM[0] > 1:
	...
	if "meta" in dados2:
		if MUfanArt:
			...
			try:
				...
				pass
		if 'season' in dados2 and 'episode' in dados2:
			infol["season"] =  int(dados2["season"])
			infol["episode"] =  int(dados2["episode"])
			infol['mediatype'] = "episode"
			infol["title"] = re.sub('\d{1,2}x\d{1,3}. ', '', xbmc.getInfoLabel("ListItem.Label"))
			try:
				if CtraktAuth and CTraktResume:
					RSM = trakt.TraktResumeS(dados)
					if RSM[0] == -1:
						d = xbmcgui.Dialog().yesno("CubeTor", "Não foi possível resgatar o progresso, reproduzir assim mesmo?")
						if not d:
							return
				...
				pass
		if 'forcemg' in dados2:
			listitem.setLabel2("c")
			mm = mg.get_episode_details(dados2['meta']['tmdb_id'], SEAS(dados2['season']), EPI(dados2['episode']), ignore_cache=True, lang=MUlang)
			if not mm:
				mm['season'] = int(dados2['season'])
				mm['episode'] = int(dados2['episode'])		
			#ST(mm)
			infol = mergedicts(dados2["meta"],mm)
			#if not mm:

			infol['mediatype'] = "episode"
			try:
				listitem.setLabel(infol["EpisodeTitle"])
			except:
				pass
			infol.pop('cast', 1)
			infol.pop('genre', 1)
...
def AddDir(name, url, mode, iconimage='', logos='', info='', dados={}, isFolder=True, IsPlayable=False):
	...
	if 'meta' in dados:
		if 'season' in dados and 'episode' in dados:
			...
			liz=xbmcgui.ListItem(name)
			if 'pc' in dados:
				dados['meta']['playcount'] = 1 if dados['pc'] else 0
			#dados['mediatype'] = "episode"
			#ST(dados)
	...
	if IsPlayable:
		#liz.setProperty('ProgressPercent', '50')
		#liz.setProperty('StartPercent', '50')
	...
	if mode == "tmdb.Opcoes":
		...
		#liz.addContextMenuItems(items = [])
	if mode == "trakt.Shows" or mode == "trakt.ShowsNext" or mode == "trakt.Seasons":
		liz.addContextMenuItems(items = [("[COLOR green]Recomendações[/COLOR]", 'RunPlugin({0}?mode=tmdb.tvshow_recom&url={1}&dados={2})'.format(sys.argv[0], quote_plus(url), quote_plus(str(dados)) )),
...
def traktT(id):
	...
	try:
		...
		request = Request('https://api.trakt.tv/shows/'+id+'/progress/watched', headers=headers)
		response_body = urlopen(request).read()
		entries=json.loads(response_body)
		trak = []
		#ST(entries["seasons"])
		for entry in entries["seasons"]:
			watched = -1
			for entry2 in entry["episodes"]:
				#for entry3 in 
				if entry2["completed"] == True:
